...
地震のときは震源(`epi`)を渡すと、その位置に ✕ マークと「震度/マグニチュード」が入ります。

```
GET /images/alert.webp?pref=43:purple&pref=44:orange&epi=32.7545,130.762&int=7&mag=7.3
```
...
| `int` | 最大震度。`1`〜`4` / `5-` / `5+` / `6-` / `6+` / `7`(`5弱` のような漢字表記も可)。`epi` と併用 |
| `over` | `1` を渡すと「震度5弱**以上**」と描く。緊急地震速報で予想震度の上限が決まらないとき(電文の `forecastMaxInt.to = "over"`)用。`int` と併用 |
| `mag` | マグニチュード(0〜10)。`7.3` でも `M7.3` でも可。表示は小数1桁に揃える。`epi` と併用 |
| `w` / `h` | 画像サイズ(100〜2000)。既定は OGP 向けの 1200×630 |
| `view` | `auto`(既定): 発令県と震源が収まる範囲へ自動ズーム / `japan`: 全国図固定 |

`int` / `mag` はラベルを ✕ に添える形なので、`epi` なしで指定するとエラーになります。
予想震度が「不明」のときは `int` を省略してください(✕ と `M` だけになります)。
ラベルは ✕ の右→左→下→上 の順に、画像に収まって凡例と重ならない位置へ置きます。

> `int=5+` の `+` はクエリ文字列では空白にデコードされますが、末尾の空白は `+` として
> 解釈するので `%2B` にエスケープしなくても通ります。
